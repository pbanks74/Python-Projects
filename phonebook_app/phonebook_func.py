...
import sqlite3
# Imports other modules to have access to them:
import phonebook_main
import phonebook_gui


def center_window(self, w, h) : # Passes in tkinter frame(master) reference and the w and h
...
# Catches if the user clicks on the upper right window "X" to ensure they want to close:
def ask_quit(self) :
    if messagebox.askokcancel("Exit program", "Okay to exit application?") :
        # Closes app:
        self.master.destroy()
        os._exit(0) # Fully removes all widget references from user's memory - prevents "memory leak"
...
def create_db(self) :
    ...
    with conn :
        ...
        # commit() saves changes & closes the database conection--*MUST DO*
        conn.commit()
    conn.close()
    first_run(self)
...
def onSelect(self,event) :
    ...
    with conn :
        ...
        # Returns a tuple and it can be sliced into 4 parts using data[] during the iteration
        for data in varBody :
...
def addToList(self) :
    ...
    if not "@" or not "." in var_email: # using in the future
        print("Incorrect email format!!!")
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0) : # Enforces user to provide
        conn = sqlite3.connect('phonebook.db')
        with conn :
            cursor = conn.cursor()
            # Checks the database for existance of the fullname, if so will alert user and disregard request
            ...
            if chkName == 0: # if this is 0 then there is no existance of the fullname and we can add new data
                ...
                messagebox.showerror("Name Error"," '{}' already exists in the database! Pleased choose a different name.".format(var_fullname)
                onClear(self) # calls the function to clear all the text boxes
        conn.commit()
        conn.close()
        else :
...
def onDelete(self) :
    ...
    with conn :
        cur = conn.cursor()
        # Checks the count to ensure that this is not the last record in database...cannot delete last record or error will occur
        cur.execute("""SELECT COUNT (*) FROM tbl_phonebook""")
        count = cur.fetchone() [0]
        if count > 1 :
            ...
            if confirm :
                ...
                onDeleted(self) # calls the function to clear all of the text boxes and the selected index of listbox
#####       onRefresh(self) # updates the listbox of the changes
                conn.commit()
        else :
            confirm = messagebox.showerror("Last record Error", "({}) is the last record in the database and cannot be deleted at this
    conn.cllose()


def onDeleted(self) :
    # Clears the text in the following textboxes
    self.txt_fname.delete(0,END)
    self.txt_lname.delete(0,END)
    self.txt_phone.delete(0,END)
    self.txt_email.delete(0,END)
...
def onUpdate(self) :
    try :
        ...
        return
    # User will only be able to update changes for phone and emails.
    # For name changes, the user will need to delete the entire record and start over.
    var_phone = self.txt_phone.get().strip() # Normalizes data to maintain database integrity
    var_email = self.txt_email.get().strip()
